...
console.log("The server is runninggggg")
//Variables-
let Users={}
let correctAnsUsers={}
let noofusers=0
...
io.on('connect',(socket)=>{
    ...
    socket.on('correctGuess',(time)=>{
        ...
        console.log('guesss correct user',Users[socket.id].name)

        // points logic 
        if (timer>50){
            Users[socket.id].points+=20;
        }
        else if(timer>40){
            Users[socket.id].points+=40;
        }
        else if(timer>30){
            Users[socket.id].points+=70
        }
        else if(timer>20){
            Users[socket.id].points+=100
        }
        else if(timer>0){
            Users[socket.id].points+=150;
        }
        ...
        socket.broadcast.emit('rightAnswer',{rightAnsUsers:correctAnsUsers,updatedUsers:users})
